...
from .sessions import (
    extract_all as extract_sessions,
    get_sessions_dir,
    load_index,
...
@main.group()
def sessions() -> None:
    """Extract and inspect OpenClaw session transcripts."""
    pass
...
@click.pass_context
def sessions_extract(ctx: click.Context, output_dir: str | None, output_limit: int, force: bool) -> None:
    """Extract OpenClaw sessions containing clawpm tool calls.

    Creates .jsonl (machine) and .md (human) files per session.
    Tracks extracted sessions in index.jsonl to skip duplicates.
    """
    fmt = get_format(ctx)
    ...
    if not sdir:
        output_error(
            "no_sessions",
            "OpenClaw sessions directory not found (~/.openclaw/agents/main/sessions/)",
            fmt=fmt,
        )
        sys.exit(1)
    ...
    if fmt == OutputFormat.JSON:
        ...
        if extracted:
            click.echo(f"Extracted {len(extracted)} session(s) to {out}/")
            for r in extracted:
                click.echo(f"  + {r['session_id'][:8]}... "
                           f"{r['clawpm_calls']} clawpm calls, "
                           f"{r['entry_count']} entries "
                           f"({r['session_start'][:10]})")
        if skipped:
            click.echo(f"Skipped {len(skipped)} already-extracted session(s)")
        if not extracted and not skipped:
            click.echo("No sessions with clawpm tool calls found.")
...
def sessions_list(ctx: click.Context, output_dir: str | None, processed: bool) -> None:
    """List extracted sessions from the index."""
    from .sessions import get_processed_ids
    ...
    processed_ids = get_processed_ids(out)

    if processed:
        # Show processed sessions
        if not processed_ids:
            if fmt == OutputFormat.JSON:
                output_json({"sessions": [], "count": 0})
            else:
                click.echo("No processed sessions.")
            return

        if fmt == OutputFormat.JSON:
            output_json({
                "sessions": sorted(processed_ids),
                "count": len(processed_ids),
            })
        else:
            click.echo(f"{len(processed_ids)} processed session(s):\n")
    ...
    if not index:
        if fmt == OutputFormat.JSON:
            output_json({"sessions": [], "count": 0, "processed": len(processed_ids)})
        else:
            click.echo("No extracted sessions. Run 'clawpm sessions extract' first.")
            if processed_ids:
                click.echo(f"({len(processed_ids)} session(s) already processed)")
        return

    entries = sorted(index.values(), key=lambda e: e.session_start)
    ...
    if fmt == OutputFormat.JSON:
        ...
        click.echo(f"{len(entries)} extracted session(s):")
        if processed_ids:
            click.echo(f"({len(processed_ids)} already processed)\n")
        else:
            click.echo("")
        for e in entries:
            click.echo(f"  {e.session_id[:8]}...  "
                       f"{e.session_start[:10]}  "
                       f"{e.clawpm_call_count:3d} clawpm calls  "
                       f"{e.entry_count:4d} entries")
...
def sessions_process(ctx: click.Context, session_ids: tuple[str, ...], output_dir: str | None, process_all: bool) -> None:
    """Mark sessions as processed (moves to processed/ subdirectory).

    Pass session IDs (or prefixes) to mark specific sessions,
    or use --all to process everything in the extracted directory.
    """
    import shutil
    ...
    if process_all:
        targets = [f.stem for f in out.glob("*.jsonl") if f.name != "index.jsonl"]
    elif session_ids:
        # Support prefix matching
        all_extracted = [f.stem for f in out.glob("*.jsonl") if f.name != "index.jsonl"]
        targets = []
        for sid in session_ids:
            matches = [s for s in all_extracted if s.startswith(sid)]
            targets.extend(matches)
    else:
        output_error("no_sessions", "Specify session IDs or use --all", fmt=fmt)
        sys.exit(1)

    if not targets:
        output_error("no_matches", "No matching sessions found", fmt=fmt)
        sys.exit(1)
    ...
    for sid in targets:
        for ext in (".jsonl", ".md"):
            src = out / f"{sid}{ext}"
            dst = processed_dir / f"{sid}{ext}"
            if src.exists():
                shutil.move(str(src), str(dst))
        moved.append(sid)

    # Update index: remove processed sessions
    index_path = out / "index.jsonl"
    index = load_index(index_path)
    for sid in moved:
        index.pop(sid, None)
    from .sessions import write_index
    ...
    if fmt == OutputFormat.JSON:
        ...
        click.echo(f"Processed {len(moved)} session(s) → {processed_dir}/")
        for sid in moved:
            click.echo(f"  → {sid[:8]}...")
